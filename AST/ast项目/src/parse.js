...
export default function (templateString) {
    ...
    var startRegExp = /^\<([a-z]+[1-6]?)(\s[^\<]+)?\>/;   // 开始标记 (\s空白字符)
    var endRegExp = /^\<\/([a-z]+[1-6]?)\>/;   // 结束标记
    // 点.	查找单个字符，除了换行和行结束符; [^] 除了
    var wordRegExp = /^([^\<]+)\<\/[a-z]+[1-6]?\>/;   //结束标记前的文字(除了<的内容)

    while (index < templateString.length - 1) {
        // console.log(templateString[index]);
        rest = templateString.substring(index);  //剩余部分

        if (startRegExp.test(rest)) {            //开始标记
            let tag = rest.match(startRegExp)[1];  //开始标记内容
            let attrsString = rest.match(startRegExp)[2];
            stack1.push(tag);   // 将tag推入stack1
            // 将对象推入stack2  (attrs: [{ name: "class", value: "box" }],)
            ...
            let pop_tag = stack1.pop();    //弹出开始标记内容
            // 此时，tag一定是和栈1顶部的是相同的
            if (tag == pop_tag) {   //若开始结束标记内容相同
                // 将stack2中的文字弹栈
